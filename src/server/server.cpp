...
void Server::run() {
    ...
    while (true) {
        ...
        m_receive_buffer.append(buffer, bytes_read);
        std::cout << "server received: '" << std::string_view(buffer, bytes_read) << "'\n";
        ...
        while (true) {
            ...
            switch (result.status) {
                ...
                case Message::Parser::Status::Invalid:
                    ...
                    m_receive_buffer.clear(); // discard malformed data to avoid repeated reparsing
            ...
            if (result.status == Message::Parser::Status::Invalid)
                break;
        }
    }
}
...
void Server::accept_client() {
    ...
    if ((m_client_fd = accept(m_server_fd, reinterpret_cast<struct sockaddr*>(&m_addr), &addr_len)) < 0) {
...
void Server::send_response(const Command::Result& result) {
    std::string response;
